...
- **Version realignment**: reset this public mirror's version from 0.2.0 to 0.1.1 so it tracks the private source of truth (`TheMizeGuy/scrum-master`). Public mirror versions now move in lockstep with the private plugin; the earlier independent public-milestone numbering (0.2.0) is retired. No functional or content change — this bump only aligns the version number.
