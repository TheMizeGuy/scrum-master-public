...
description: |-
  ...
  Do NOT use for actual story implementation (writing code) — that's the dev plugin or manual work.
...
**Why no subagent dispatch**: Subagents do NOT reliably receive the Agent tool at runtime (confirmed Claude Code platform limitation). The scrum-master workflow dispatches read-only scout runners for reconnaissance — a subagent wouldn't have Agent tool access and couldn't dispatch scouts. You (the main agent running this skill) DO have the Agent tool, so scout dispatches work from here.
...
4. Execute the mode flow directly (dispatching scouts yourself, reading/writing files, making decisions)
...
This is your operating manual. It defines the 13 modes, story schema, scout protocol, board view format, error handling, and every decision rule. Follow it as written — you ARE the scrum-master.
...
- **Dispatch scouts from here** (the main agent context has the Agent tool). Use `Agent({ description: "...", prompt: "..." })` — omit `model`; scouts inherit the session model (always the strongest available Claude) — with the scout prompt templates from the system prompt. Do NOT use `subagent_type` — general-purpose is what provides the read-only tools the scouts need.
...
Scouts inherit the session model — omit `model` on every dispatch; there is no separate "scout model" to fall back to or wait on. If the session model is already the strongest available tier and a mode's recon is small (e.g., a single board scan), read the files directly instead of dispatching a scout — reserve scout dispatch for genuinely parallel or bulk recon. Never block on, or call out to, a model that isn't the session model. This does not relax the read-only/owns-all-writes split: whether recon runs as a dispatched scout or inline, only you (the main agent) ever write, edit, or delete a story or board file.
